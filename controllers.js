...
async function all(file) {
    ...
    for (let i of listSupplier) {
        let data = {
            name: i.name,
            data: await perhitungan(i.data, i.name),
        };
        
        result += `${moment().format("DD/MM/YY hh:mm")}\n${data.name}:\n${data.data}\n,\n`;
    }


    fs.writeFileSync(path.join(__dirname, `./public/dream_beach_report.csv`), result);
}
